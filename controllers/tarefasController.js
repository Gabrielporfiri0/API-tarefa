...
exports.criarTarefa = async (req, res) => {
  const { titulo, preco, categoria, data_criacao } = req.body;

  try {
    const resultado = await db.query(
      'INSERT INTO tarefas (titulo, preco, categoria, data_criacao) VALUES ($1, $2, $3, $4) RETURNING *',
      [titulo, preco, categoria, data_criacao]
    );
    res.status(201).json(resultado.rows[0]);
  } catch (err) {
